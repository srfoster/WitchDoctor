 ...
 *******************************************************************************/
package mychangedetector.copyclasses;
...
public class MyDifferencer {
	...
	static class Node {
		...
		Object visit(MyDifferencer d, Object parent, int level) {
			if (fCode == NO_CHANGE)
				return null;
			//dump(level);
	...
	private int traverse(boolean threeWay, Node parent, IProgressMonitor pm, Object ancestor, Object left, Object right) {
		...
		if (rightChildren != null && leftChildren != null) {
			...
			while (e.hasNext()) {
				...
				int c= traverse(threeWay, node, pm, null, leftChild, rightChild);
