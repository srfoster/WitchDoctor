 ...
 * A Java code scanner.
 */
public class MyJavaCodeScanner extends AbstractJavaScanner {
	...
	private static class GhostTextRule implements IRule {
		 ...
		 * @param token Token to use for this rule
		 */
		public GhostTextRule(IToken token) {
			fToken= token;
		}
		...
		public IToken evaluate(ICharacterScanner scanner) {
			...
			int character= scanner.read();
			if (isWithinGhostMarkers((char) character, (MyJavaCodeScanner) scanner)) {
				do {
					character= scanner.read();
				} while (isWithinGhostMarkers((char) character,(MyJavaCodeScanner) scanner));
				scanner.unread();
				return fToken;
			} else {
				scanner.unread();
	 ...
	 */
	public MyJavaCodeScanner(IColorManager manager, IPreferenceStore store) {
		super(manager, store);
		initialize();
	}
